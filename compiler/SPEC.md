...
（注意：char/character **类型**的语义与运行期支持尚不完整，见缺口 G2。）
...
8. 乘除模 `* / %`
9. 一元 `- !`
10. 后缀链：索引 `a[i]`、方法调用 `x.m(args)`、属性 `x.p`（可任意混合链式）
11. primary：字面量、标识符、`(expr)` 分组、元组字面量、`new 类名(args)`、`this`、
    `base.method(args)`、数组字面量
...
| 编号 | 缺口 | 现状 | 计划 |
|------|------|------|------|
| G1 | **位运算符未解析**：`& \| ^ ~ << >>` 仅词法识别，parser 无对应优先级层 | 写位运算表达式报语法错误 | t47 |
| G2 | **char/character/bit/byte/word/dword 类型不完整**：可声明（语义放行），但解释器 Value 层无对应表示，字符字面量运行期落为 string | 相关 DISABLED_ 测试挂起 | t47（char/byte 优先） |
