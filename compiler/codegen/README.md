...
| 范围外 | 非 Obj 初始值（string/number/none 等需动态值表示，官方示例 main.collie 的 object 用法属此，实证 p4 拒编）；无初始化 `object x;`（可赋任意值语义边界同前）；兄弟类/跨树重赋走 visitAssign 既有守卫拒编不错编（实证 p3）；零新增 collie_rt 接口 |
