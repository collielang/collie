...
最后更新：2026-08-01（t99 完成：codegen 类方法/构造器 byte/word 参数与返回放行——方法注册循环前置识别返回类型/参数为 Int 承载 + CGMethod.ret_bit_max，gen_method_body 设 current_ret_bit_max_（返回走 t97 陷阱）+ 形参绑定点插 check_bit_range 范围陷阱（方法单签名按名解析，实参可为整数字面量，覆盖方法/构造器/base 全路径）+ 置 CGVar.bit_max，coerce_call_arg 零改动，差分 51/51）
