...
最后更新：2026-08-01（t100 完成：codegen 类实例进数组（同类，kind 5）放行——arr_kind_of(Obj)→5、elem_to_bits/bits_to_elem 加 PtrToInt/IntToPtr，visitArrayLiteral 追踪元素类名（复用 CGValue.cls）+ 混合类守卫，visitVarDecl/visitAssign array 分支传播 cls + 异类守卫，visitIndex 静态读出传播 cls（arr[i].field/method() + 动态分派），visitIndexAssign Obj 分支 cls 兼容守卫（子类 upcast 放行），rt 三触点加 kind 5——arr_to_str `<object>` / arr_eq 恒不等 / trap_arr_kind `object`；build/len/print/toString/==/整槽写/本地静态读出全解锁，动态域 obj 读出落 CG9 陷阱不错编，差分 52/52）
