...
最后更新：2026-08-01（t98 完成：codegen byte/word 函数参数放行——declare_function 参数循环前置识别为 Int 承载，visitFunction 绑定置 CGVar.bit_max 复用赋值点范围陷阱，调用点无需陷阱（重载解析保证实参恒为已校验 byte/word 值），类方法/构造器参数仍拒编，差分 50/50）
