...
最后更新：2026-07-29（t84 完成：codegen 非常量 tuple get() 动态键——同质命名 tuple（元素同 CGType 且 ∈ {Int/Double/Bool/Str}、≥1 非空名）的 `t.get(k)`（k 为运行期字符串）物化 names+values 数组后新增 rt 接口 collie_rt_tuple_get 按非空名 strcmp 查找，未命中陷阱 "Undefined tuple field '<key>'" 与解释器核心消息一致，差分 36/36）
