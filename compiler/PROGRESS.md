...
最后更新：2026-08-01（t101 完成：codegen 无初始化 array/类类型变量声明放行——visitVarDecl 无初始化分支加两分支：KW_ARRAY 建 opaque ptr 槽 + elem=Num 动态域哨兵（t70），IDENTIFIER 已注册类名建 Obj 槽 + cls；visitAssign Arr/Obj 两分支各补同块 uninit 清除（原早退未经通用路径 t92 清除逻辑）；复用 uninit + decl_depth 机制，同块赋值后可读、深层块保守拒编；无初始化 Tuple 维持拒编（形状无从推断），动态域 obj/kind≥2 数组元素读出维持 CG9 陷阱，差分 53/53）
...
- [x] codegen Num 元素数组字面量（t90）
    - 选型：剩余候选预检实证——候选 C（位运算/移位/位取反 Num 整数态）排除：语义层 "Bit operands expected for bitwise operation" 两端一致拦截（预检 7 错误双端同报），非差分面；候选 B（Num 元素数组字面量）成立：解释器 `number x=7; number y=2.5; array a=[x,y,3]` print/索引读写/length 5 行全可跑 vs colliec 拒编 "heterogeneous array literal"，活跃差分面；候选 D（无初始化变量绑 none，解释器 declared/42 可跑 vs colliec 拒编）成立但涉变量槽类型流转（None→声明类型）规模较大后置
    - 范围拍板：(1) visitArrayLiteral 数值系同质判定扩展——元素含 Num 时（{Int/Double/Num} 互混或全 Num）elem 统一提升 Double（Num 运行期 tag 静态不可判，double 视图；rt format_f64 整数值省 .0 与解释器混合表示输出对齐，Int→Double 混合提升既有先例）；(2) to_double 加 Num 分支（tag==0 ? SIToFP(bits) : bitcast(bits)，select 无副作用）；(3) visitIndexAssign 静态数值系槽（elem∈{Int,Double}）收 Num 值下沉既有 rt_arr_set_num（tag==kind 直存/0→1 提升/1→0 落 CG7 陷阱，对齐槽同质表示）；范围外：超大整数 Num 进数组 double 视图丢精度（Int→Double 混合提升既有同域先例，CG1 缺口范畴）、无初始化变量（候选 D）；零新增 collie_rt 接口
    - 实现：code_generator.cpp 三处——visitArrayLiteral 同质判定 numlike lambda（{Int/Double/Num} 互混提升 Double）+ 循环后 elem==Num 兜底落 Double + 存槽循环 Int/Num 元素 to_double 提升；to_double 加 Num 分支（num_tag/num_bits 提取后 SIToFP/BitCast select 合流）；visitIndexAssign 静态路径加 Num 值分支（elem∈{Int,Double} 时 num_tag/num_bits 下沉 rt_arr_set_num）
    - 验证：s43 差分 16 行逐字节一致（Num 整/小数态与 Int/Double 混合字面量/全 Num 字面量/print/索引读含负索引/length/len/Num 值写 int-double 槽/深比较 3 例/函数内局部 Num 数组循环遍历两态）；陷阱实证 trap1（小数态 Num 写 int 槽 ia[0]=1.5——解释器 before+[1.5, 2] vs 产物 CG7 "array element type mismatch"）；差分 42/42 + Debug 6/6
    - 范围外：超大整数 Num 进数组 double 视图丢精度（CG1 缺口范畴）；无初始化变量绑 none（候选 D 后置）；零新增 collie_rt 接口
- [x] codegen 嵌套函数声明（t91）
    - 选型：剩余候选预检实证——候选 E（非 bool if/while 条件）排除：语义层 "If/While condition must be a boolean expression" 两端一致拦截（预检 4 错误双端同报），非差分面；候选 F（嵌套函数声明）成立：解释器 `function outer() integer { function inner() ... return inner()+1; }` 输出 6 vs colliec 拒编 "nested function declaration"，活跃差分面；解释器语义为执行到声明处 env_.define 动态绑定（动态作用域下内层可捕获外层局部，预检 G 输出 42 确认）；候选 D（无初始化变量）继续后置
    - 范围拍板：受限雷姆达提升——(1) 第一遍原型建立递归下探函数体内的 FunctionStmt，嵌套函数以改编名（外层名.内层名）建模块级原型，进独立注册表按声明语句指针索引；(2) visitFunction 解除 in_function_ 拒编，嵌套声明处将改编原型登记进当前作用域层（对齐解释器"执行到声明处才可见"+ 遮蔽 + 块退出失效），函数体生成复用既有独立现场机制；(3) 调用点解析先查作用域链局部函数绑定再查顶层 functions_；范围外：内层函数引用外层局部变量（标识符不可见拒编不错编——静态提升无捕获机制，捕获面后置）、同名嵌套函数遮蔽顶层同名（作用域链天然支持则放行）；零新增 collie_rt 接口
    - 实现：code_generator.h 三处——CGVar 加 fn_key 字段（非空即嵌套函数绑定）、declare_function 加 prefix 缺省参 + declare_nested_in 声明、nested_fns_ 成员（FunctionStmt* → 改编键）；code_generator.cpp 六处——generate() 清 nested_fns_、declare_function 改编键（prefix.name + 符号 collie.key）尾部递归 declare_nested_in、新增 declare_nested_in（dynamic_cast 分派 Block/If/While/For/DoWhile/Switch 下探）、visitFunction 嵌套路径（查表区分 + 声明处 scopes_ 登记 fn_key 绑定 + 现场 in_function_/返回类型保存恢复 + 链底拷入外层函数绑定）、visitCall 作用域链函数绑定优先、visitIdentifier/visitAssign fn_key 守卫拒编
    - 验证：s44 差分 5 行逐字节一致（基本嵌套 6/嵌套读全局 22/自身递归 fac(5)=120/前置兄弟嵌套 8/带参字符串嵌套 [collie]）；拒编实证 3 例——捕获外层局部（解释器 42 vs "identifier 'captured'"）、类方法体内嵌套（"nested function declaration"）、函数名作值（"function 'f' used as a value"）；差分 43/43 + Debug 6/6
    - 范围外：嵌套体引用外层局部（捕获面拒编不错编）；类方法体内嵌套函数维持拒编；函数名作值/被赋值拒编（非一等公民）；同外层同名嵌套语义层 "already defined" 双端拦截非差分面；零新增 collie_rt 接口
- [x] codegen 无初始化变量声明（t92）
    - 选型：候选 D（两轮后置）预检实证——预检 A：四静态类型 `integer x; ... x = 42; print(x)` 解释器 5 行可跑 vs colliec 拒编 "variable declaration without initializer"，活跃差分面；预检 B：未赋值先读被语义层 "used before initialization" 双端拦截；预检 C（关键）：条件分支内赋值后读语义层流不敏感放行、解释器运行期打印 none——codegen 若零初始化槽会打印 0，错编风险，须自建保守守卫
    - 范围拍板：(1) visitVarDecl 无初始化放行限四静态类型 {integer,decimal,bool,string}——槽照常创建（顶层零初始化全局槽/函数内 alloca 不预存），CGVar 加 uninit 标记 + decl_depth（声明时 scopes_.size()）；(2) visitIdentifier 读 uninit 槽拒编（预检 C 面拒编不错编）；(3) visitAssign uninit 槽赋值走既有 coerce_for_slot 链路，scopes_.size()==decl_depth（同块直线区域，运行期必先于后续读执行——同深度即同块：块被弹出则变量记录随之消亡）时清 uninit，深层块赋值存值不清标记（后续读仍拒编保守安全）；范围外：byte/word/number/tribool/array/Tuple/类类型无初始化维持拒编、分支/循环块内赋值后读拒编不错编、函数体快照见 uninit 全局槽保守拒编；零新增 collie_rt 接口
...
- [x] codegen 三元/==? 分支数组元素类型合流统一动态域（t94）
    - 选型：剩余拒编面盘点——预检 A（`c ? a : b` 两支数组 elem 不同 Int/Double，索引读）解释器 `1`/`2.5` vs 拒编 "ternary branches yield arrays of different element types"，活跃差分面（t93 实例合流的姊妹面）；预检 B（数组变量再赋不同 elem `a = [1.5, 2.5]` 后读）解释器可跑 vs 拒编 "assigning array with different element type"——同为活跃面但排除后置：程序序提升 var->elem 在循环回边（先读后赋再回读）与函数全局快照静态解码下会错编，需循环深度守卫+捕获跟踪，复杂度不匹配
    - 范围拍板：gen_ternary 与 match 合流两触点 Arr elem 不等时改统一 elem=Num 动态域哨兵（t70/t88 既有机制）——数组值不透明 ptr，PHI 无关 elem；合流值为新鲜值，元数据自诞生即动态无程序序失配，kind 随数组对象运行期自带：print/len/== rt 侧全 kind 覆盖，索引读数值系正常、kind ≥ 2 落既有 CG9 陷阱不错值；两触点引入 result_elem 逐支累计（同 t93 result_cls 模式）；范围外：数组变量再赋不同 elem 维持拒编（后置候选）；零新增 collie_rt 接口
...
- [x] codegen 无初始化变量声明扩展 number/tribool/byte/word/char/character（t96）
    - 选型：剩余拒编面盘点——预检 A（`number n; n = 42;` 后读 + tribool/byte/word 同形）解释器 `42/2.5/unset/200/60000` vs 拒编 "variable declaration without initializer"，活跃差分面（S45 t92 uninit 机制的自然延伸，纯静态零错编风险），胜出；预检 B（char/character 无初始化）解释器 `x/y` vs 同消息拒编，一并纳入；异质数组字面量/实例进数组/tuple 进签名维持后置（t95a 已实证记录）
    - 范围拍板：visitVarDecl 无初始化放行面从四静态类型扩展到 {number,tribool,byte,word,char,character}——Num（struct{i64,i64}）/Tri（i8）/Str（ptr）零初始化槽合法且 uninit 期读拒编不触及零值；byte/word 沿用 i64 承载 + CGVar.bit_max，既有 visitAssign 赋值点 check_bit_range 陷阱自动生效；uninit/decl_depth/同块清除机制不动；范围外：array（elem 无从推断）/Tuple（形状无从推断）/类类型维持拒编，分支/循环块内赋值后读维持拒编（同 t92）；零新增 collie_rt 接口
    - 实现：code_generator.cpp visitVarDecl 无初始化分支——byte/word 前置分支（i64 槽 + bit_max + uninit）；四静态类型放行条件扩展 KW_NUMBER/KW_TRIBOOL/KW_CHAR/KW_CHARACTER（复用 declared_cgtype + llvm_type_of 零初始化槽）
    - 验证：新差分用例 s49_uninitmore（number 整/小数态、tribool 三态、byte/word 范围内赋值、char/character、函数内局部 number、顶层全局 tribool 函数体内读、循环体内声明+同块赋值 number，16 行输出）双端逐字节一致；两实证——array 无初始化维持拒编（解释器 `1` vs 拒编 "variable declaration without initializer"）、byte 分支块内赋值后读维持拒编（解释器 `5` vs 拒编 "use of uninitialized variable"）；ctest -C Release 差分 48/48，Debug 门禁 6/6
    - 范围外：array/Tuple/类类型无初始化维持拒编；分支/循环块内赋值后读维持拒编不错编；零新增 collie_rt 接口
- [x] codegen byte/word 函数返回类型（t97）
    - 选型：剩余拒编面盘点——探针 p2（byte/word 作函数返回类型 `function makew() word { return 40000; }`）解释器可跑输出 `60000/40000` vs codegen 拒编 "variable type 'word'"，活跃差分面，胜出；越界返回探针 p3（`return 70000` 于 word 返回函数）解释器报 "Value out of range for 'word' (expected 0-65535, got 70000)"——确认解释器返回值走 coerce_to_declared 范围校验，check_bit_range 陷阱核心消息对齐可保拒编不错编；探针 p1（byte 作函数参数，正确语法 `function addb(b byte)`）解释器可跑输出 `201` vs codegen 拒编 "variable type 'byte'"——同为活跃差分面但排除后置：参数位需 CGFunction 存 per-param bit_max、visitFunction 绑定时置 CGVar.bit_max（体内重赋走赋值点检查）、并在 coerce_call_arg 调用点补范围陷阱（否则 `addb(300)` 越界实参解释器 trap 而 codegen 直传会错编），触点多于返回位，本任务先落返回位；异质数组字面量/实例进数组/tuple 进函数签名三活跃差分面维持后置（t95a/t96a 已实证记录）
    - 范围拍板：declare_function 顶层函数返回类型 KW_BYTE/KW_WORD 前置识别为 CGType::Int 承载 + CGFunction 新增 ret_bit_max（255/65535，缺省 0 保兼容），visitFunction 保存/设置/恢复 current_ret_bit_max_，visitReturn 值对齐 Int 后 ret_bit_max>0 时插 check_bit_range（复用 t69 既有陷阱，越界调 rt_trap_bit_range 报错退出，对齐解释器 coerce_to_declared 返回值校验）；返回值消费端为普通 Int 表达式域（存 byte 变量再走赋值点检查、print 直接整数输出）；范围外：byte/word 作函数参数（活跃差分面，后置——需 per-param bit_max + 调用点范围陷阱）、类方法返回 byte/word（走 declared_signature_type 维持拒编）；零新增 collie_rt 接口
    - 实现：code_generator.h CGFunction 加 ret_bit_max 字段 + 新增 current_ret_bit_max_ 成员；code_generator.cpp declare_function 顶层函数返回类型 KW_BYTE/KW_WORD 前置识别为 CGType::Int + ret_bit_max=255/65535（绕过 declared_signature_type）、functions_ 初始化带入 ret_bit_max、visitFunction 保存/设置/恢复 current_ret_bit_max_、visitReturn 值对齐 Int 后 current_ret_bit_max_>0 时插 check_bit_range（多返回路径逐 return 各插）
    - 验证：新差分用例 s50_bytewordret（byte/word 返回值 print、参与算术、存 byte/word 变量走声明与赋值点范围陷阱、clampToByte 多返回路径、循环内反复调用、word 返回值 == 比较，12 行输出 `200/60000/210/200/255/255/42/60100/200/201/202/wordok`）双端逐字节一致；两实证——越界 word 返回探针 p3（`return 70000`）codegen 产物运行期 trap "Value out of range for 'word' (expected 0-65535, got 70000)"（核心消息对齐解释器）、byte 作函数参数探针 p1（`addb(b byte)`）解释器 `201` vs codegen 拒编 "variable type 'byte'" 活跃差分面后置实证；ctest -C Release 差分 49/49，Debug 门禁 6/6
    - 范围外：byte/word 作函数参数（活跃差分面，后置——需 per-param bit_max + 调用点范围陷阱）、类方法返回 byte/word 维持拒编；异质数组/实例进数组/tuple 进签名后置；零新增 collie_rt 接口
- [x] codegen byte/word 函数参数（t98）
    - 选型：t97 后置候选 byte/word 作顶层函数参数——预检实证：p3（`byte x = 200; print(addb(x))`，addb 形参 byte）解释器 `201` vs codegen 拒编 "variable type 'byte'"，活跃差分面胜出；关键实证：p1/p2（`addb(200)`/`addb(300)` 整数字面量实参）与 p4（`addb(x + 100)` byte 算术实参）双端均报语义错 "No matching overload for function 'addb'"——重载解析要求实参类型即 byte/word，整数字面量/整数算术表达式不匹配，非差分面（无需处理）；由此推出核心结论：byte/word 形参只能接收 byte/word 类型实参（byte/word 变量已在声明/赋值点 check_bit_range、byte/word 函数返回已在 visitReturn t97 check_bit_range），实参恒在范围内，**调用点无需补范围陷阱**（原 t97 记录的"调用点范围陷阱"经预检证伪，实为类型系统天然保证）；p5（`idb(a)`/`addw(m)`/`d=d+5;idb(d)` 合法 byte/word 变量实参）解释器 `10/60001/255` 待差分
    - 范围拍板：declare_function 顶层函数参数循环 KW_BYTE/KW_WORD 前置识别为 CGType::Int 承载（绕过 declared_signature_type 的 "variable type" 拒编，llvm 形参 i64）；visitFunction 形参落槽绑定时 byte/word 形参置 CGVar.bit_max（255/65535）——体内重赋走既有 visitAssign 赋值点 check_bit_range 陷阱（`b = b + 100` 越界 trap），返回 `b + 1` 越界走 visitReturn t97 陷阱；实参对齐（coerce_call_arg）零改动——实参恒为已校验的 Int 值（a.type==want==Int 直传）；范围外：类方法/构造器 byte/word 参数（走 declared_signature_type 维持拒编）、整数字面量/算术实参非差分面；零新增 collie_rt 接口
    - 实现：code_generator.cpp declare_function 参数循环 KW_BYTE/KW_WORD 前置识别为 CGType::Int（绕过 declared_signature_type，llvm 形参 i64）；visitFunction 形参落槽绑定改用具名 CGVar binding，byte/word 形参置 bit_max=255/65535；修正 t97 返回类型块过时注释（"参数位解析器不接受 byte/word"→参数循环 t98 已放行）；coerce_call_arg 零改动
    - 验证：新差分用例 s51_bytewordparam（byte 形参 identity inc、clamp 多返回、word 形参 addw、嵌套调用 chain、byte 函数返回作实参 inc(makeByte())、循环内反复以 byte 变量为实参、形参返回值 == 比较、word 返回参与算术，9 行输出 `51/50/200/60001/51/101/3/ok/106`）双端逐字节一致；实证——p6 形参体内重赋越界（byte 形参内 `b = b + 100`，实参 200）解释器 trap "Value out of range for 'byte' (expected 0-255, got 300)" vs codegen 产物运行期 trap 核心消息对齐（走既有赋值点 check_bit_range）；ctest -C Release 差分 50/50，Debug 门禁 6/6
    - 范围外：类方法/构造器 byte/word 参数（走 declared_signature_type 维持拒编，活跃差分面后置）、整数字面量/算术实参非差分面；异质数组/实例进数组/tuple 进签名后置；零新增 collie_rt 接口
- [x] codegen 类方法/构造器 byte/word 参数与返回（t99）
    - 选型：t97/t98 双后置候选——预检实证三面均活跃差分面：q1（方法 byte 参数 `c.addb(x)`，x=200）解释器 `201` vs 拒编 "variable type 'byte'"；q2（方法 byte/word 返回）解释器 `200/60000/201` vs 拒编；q3（构造器 byte 参数 `new C(x)`，x=77，byte 字段承接）解释器 `77` vs 拒编；关键差异实证（与 t98 顶层函数不同）：q5（`c.addb(200)` 整数字面量实参）解释器 `201` 可跑——方法为单签名按名解析无重载拦截，实参经 coerce_to_declared 绑定时校验；q6/q7（`c.addb(300)`/`new C(300)` 越界字面量实参）解释器运行期 trap "Value out of range for 'byte' (expected 0-255, got 300)"（错误位置为形参声明处）——故方法/构造器 byte/word 形参需**绑定点范围陷阱**（对照 t98 顶层函数"实参恒已校验无需陷阱"）；q4（方法 byte 返回 `return 300`）解释器 trap 同 t97 返回值校验
    - 范围拍板：declare 方法注册循环两触点——返回类型 KW_BYTE/KW_WORD 前置识别为 CGType::Int + CGMethod.ret_bit_max（继承自 CGFunction，t97 已有字段）；参数循环 KW_BYTE/KW_WORD 前置识别为 CGType::Int + 新增 param_bit_max 向量（逐参 255/65535/0）；gen_method_body 两触点——形参落槽绑定 param_bit_max>0 时先插 check_bit_range（对齐解释器绑定时 coerce_to_declared 校验，统一覆盖方法/构造器/base 全调用路径）再 store 并置 CGVar.bit_max（体内重赋走赋值点陷阱）；current_ret_bit_max_ 设置/复位（visitReturn t97 陷阱自动生效）；调用点 coerce_call_arg 零改动（实参 Int 直传，范围校验在被调方绑定点）；范围外：byte/word 类字段已解锁（S40 t87）不涉及；零新增 collie_rt 接口
    - 实现：code_generator.cpp 四处——方法注册循环返回类型 KW_BYTE/KW_WORD 前置识别为 CGType::Int + info.ret_bit_max=255/65535（复用 CGMethod 继承的 t97 ret_bit_max 字段，绕过 declared_signature_type）；参数循环 KW_BYTE/KW_WORD 前置识别为 CGType::Int（llvm 形参 i64）；gen_method_body 现场保存/复位加 current_ret_bit_max_（设 method.ret_bit_max、复位 0，visitReturn t97 陷阱自动生效）；形参落槽绑定处读 stmt 形参声明类型（param.type），byte/word 时先 check_bit_range 再 store 并置 CGVar.bit_max（无需新增 param_bit_max 向量——绑定点直接由 AST 形参类型判别，较范围拍板更简）；coerce_call_arg 零改动
    - 验证：新差分用例 s52_bytewordmethod（构造器 byte 参数字面量实参 new Meter(10)、方法 byte 参数字面量+byte 变量实参 bump、方法 byte 返回 print/算术/存 byte 变量 getLevel、byte 参数+byte 返回多路径 pickByte、word 参数+word 返回多路径 clampWord、word 参数+word 返回循环内累计 addTotal、方法返回值作方法实参 m2.bump(m.getLevel())、方法 byte 返回值有序比较 >=，13 行输出 `10/15/35/35/135/35/200/0/255/200/60000/36/ok`）双端逐字节一致；两实证——构造器越界字面量实参（`new C(300)`）解释器 trap "Value out of range for 'byte' (expected 0-255, got 300)" vs codegen 产物运行期 trap 核心消息对齐、方法越界字面量实参（`c.setb(300)`）同样双端 trap 核心消息对齐（均走绑定点 check_bit_range）；用例避开解释器语义边界——方法调用结果静态类型为 object 不能直接参与 ==/!= 比较（走 >= 有序比较，object 动态放行）、word 值不能 return 给 byte 返回类型（各返回类型自洽）；ctest -C Release 差分 51/51，Debug 门禁 6/6
    - 范围外：异质数组字面量、实例进数组、tuple 进函数签名维持拒编（活跃差分面后置候选）；方法调用结果参与 ==/!= 比较、word→byte 返回属解释器语义边界（非 codegen 拒编面）；零新增 collie_rt 接口
- [x] codegen 类实例进数组（同类，kind 5）（t100）
    - 选型：前序双后置候选盘点（异质数组字面量 A / 实例进数组 B / tuple 进函数签名 C）经 Search 子代理触点核实 + 双端预检——候选 B 胜出：qb（`array arr=[d1,d2]; print(len(arr))`）解释器 `2` vs codegen 拒编 "array element type"（arr_kind_of default），活跃差分面；qb2 摸清解释器完整语义——print(arr)→`[<object>, <object>]`、arr[0]→`<object>`、arr[0].id / arr[0].who()→字段读/方法调用可用（`1`）、`[d1]==[d1]`→`false`（实例恒不等）、arr[0]=d2 整槽写生效；与 t88/t89（bool/str/嵌套数组）同构——本地 `array` 变量保留 elem=Obj（visitVarDecl elem=init.elem），读出走静态 bits_to_elem 路径（非动态陷阱），故可支持本地静态读出+字段+方法；候选 A 需 rt 每元素 tag 结构性改动且非数值元素读出仍陷阱、候选 C 需形状单态化架构级方案，均继续后置
    - 范围拍板：arr_kind_of(Obj)→5、elem_to_bits(Obj)→PtrToInt、bits_to_elem(Obj)→IntToPtr；visitArrayLiteral 全 Obj 元素追踪元素类名（复用 CGValue.cls，Arr 时表元素类）+ 混合类守卫（异类拒编 "heterogeneous array literal (mixed classes)"，后置）；visitVarDecl array 分支拷贝 cls；visitIndex 静态读出传播 cls（arr[i]→Obj+cls，支持字段读 arr[i].field、方法调用 arr[i].method()）；visitIndexAssign Obj 分支补 cls 兼容守卫（写异类拒编，允许子类 upcast 同 coerce_call_arg）；rt 三触点加 kind 5——arr_to_str（`<object>`）、arr_eq（kind 5 元素恒不等，对齐解释器 values_equal 实例恒 false）、trap_arr_kind（`object`，动态域读出兜底）；消费面 build/len/print/==/整槽写/本地静态读出全解锁，数组过签名/字段的动态域 obj 读出落 CG9 陷阱不错编；范围外：混合类实例数组、动态域 obj 元素读出维持拒编/陷阱；零新增 collie_rt 接口签名
    - 实现：code_generator.cpp 五处——三 helper（elem_to_bits(Obj)→PtrToInt / bits_to_elem(Obj)→IntToPtr / arr_kind_of(Obj)→5）；visitArrayLiteral 局部 elem_cls 追踪首元素类名 + 全 Obj 元素混合类守卫（v.cls != elem_cls 拒编）+ last_value_ 携 elem_cls；visitVarDecl array 分支拷贝 init.cls 入 CGVar；visitAssign 整数组重赋值补 Obj 异类守卫（v.cls != var->cls 拒编）+ last_value_ 携 cls；visitIndex 静态读出 last_value_ 追加 object.cls 传播（identifier load 已携 var->cls）；visitIndexAssign 静态写路径末补 Obj cls 兼容守卫（同类或子类 upcast 放行，异类拒编）；collie_rt.c 三触点——arr_to_str case 5 `<object>`、arr_eq 同 kind 5 恒返 0、trap_arr_kind kind 5→`object` + 两处头部注释登记 kind 5=object；`==`/print 按 Arr 类型泛化下沉无 elem 守卫天然覆盖
    - 验证：新差分用例 s53_objectarray（同类实例数组 build、len→2、print(zoo)/toString(zoo)→`[<object>, <object>]`、本地静态读出 zoo[i].name 字段 + zoo[0].describe()/sound() 方法调用、负索引 zoo[-1]、整槽写同类 zoo[0]=a3、子类 upcast 整槽写 zoo[1]=Dog 后读出方法动态分派 describe()→`rex says woof`、== 实例恒不等 neq/ne-true/self-neq，16 行输出双端逐字节一致）；两实证——混合类字面量（`[a, b]` 异类）解释器 `2` vs codegen 拒编 "heterogeneous array literal (mixed classes)"（line 5）活跃差分面；动态域 obj 读出（obj 数组过函数签名后 `a[0]`）解释器 `<object>` vs codegen 产物运行期 trap "reading object array element in dynamic context ... gap CG9"（trap_arr_kind kind 5→object）；ctest -C Release 差分 52/52，Debug 门禁 6/6
    - 范围外：混合类实例数组字面量/整槽写异类维持拒编，动态域（数组过签名/字段/返回值）obj 元素读出维持 CG9 陷阱不错编；零新增 collie_rt 接口签名
- [x] codegen 无初始化 array/类类型变量声明（t101）
    - 选型：Search 子代理枚举 code_generator.cpp 全部约 143 处 unsupported() 触点 + 对照 README 后置候选，核实候选 A-E——候选 D-array（无初始化 `array` 变量）+ D-class（无初始化类类型变量）胜出：双端预检实证活跃差分面——p1（`array a; a=[1,2,3]; print(...); a[0]=100; print(a[0],a[-1])`）解释器 `arr: [1, 2, 3] 3`/`idx: 100 3` vs codegen 拒编 "variable declaration without initializer"（line 2）；p2（`Cat c; c=new Cat("mimi"); print(c.name, c.speak())`）解释器 `cat: mimi mimi meow` vs codegen 同拒编（line 12）；均复用 t92/t96 uninit + decl_depth 机制，D-array 叠加 t70 Num 动态域哨兵（elem 无从静态推断）；规模 S（约 13 行同构扩展，无新 rt 接口）；候选 A（数组变量再赋不同 elem，循环回边错编需守卫，规模 M/L）、B（三元/==? 无公共祖先合流，规模 L）、C（混合类实例数组字面量，t100 已加守卫）、D-tuple（形状无从推断需延迟槽组，规模 M）均继续后置
    - 范围拍板：visitVarDecl 无初始化分支加两分支——(1) KW_ARRAY：create_var_slot(llvm_type_of(Arr)) opaque ptr 槽 + CGVar{type=Arr, elem=Num 动态域哨兵, uninit, decl_depth}（elem 无从推断用 t70 哨兵，后续赋值走动态域透传、读出 kind 驱动、kind≥2 落 CG9 陷阱）；(2) IDENTIFIER（已注册类名）：create_var_slot(llvm_type_of(Obj)) + CGVar{type=Obj, cls=类名, uninit, decl_depth}；visitAssign Arr 分支与 Obj 分支各补同块 uninit 清除（`var->uninit && scopes_.size()==decl_depth` 时 store 后清，与通用路径 t92 一致——两分支原 return 早退未清 uninit，不补则声明后赋值仍不可读）；范围外：无初始化 Tuple（形状无从推断需延迟槽组，后置）、动态域 obj/kind≥2 数组元素读出维持 CG9 陷阱、分支/循环块内赋值后读维持拒编不错编（同 t92）；零新增 collie_rt 接口
    - 实现：code_generator.cpp 三处——visitVarDecl 无初始化分支在静态类型放行块后、终点 unsupported 前加两分支：KW_ARRAY（create_var_slot(llvm_type_of(Arr)) opaque ptr 槽 + CGVar.type=Arr/elem=Num/uninit=true/decl_depth，后续赋值走 visitAssign Arr 动态域透传路径 t70），IDENTIFIER 且 classes_.count(类名)（create_var_slot(llvm_type_of(Obj)) + CGVar.type=Obj/cls=类名/uninit=true/decl_depth，后续赋值走 visitAssign Obj 同类/子类 upcast 路径 t86）；visitAssign Arr 分支与 Obj 分支 CreateStore 后各补 `if (var->uninit && scopes_.size()==var->decl_depth) var->uninit=false;`（两分支原 return 早退不经通用路径 t92 清除，不补则声明后同块赋值仍被 visitIdentifier uninit 守卫拒读）；零新增 collie_rt 接口
    - 验证：新差分用例 s54_uninitvar（无初始化 array 隔句赋值 int 数组 print/len/正负索引读写、赋 decimal 数组混合表示、别名引用语义写联动、无初始化类变量赋值后字段读+方法调用、类变量别名写字段可见、无初始化父类变量接子类实例 upcast 后方法动态分派 sound()→woof、函数内局部无初始化 array+class、for 循环体内声明+同块赋值+同块读，12 行输出双端逐字节一致）；两实证——neg1（无初始化 array 仅在 if 块内赋值后读，deep-block 保守）解释器 `neg1: [1, 2, 3]` vs codegen 拒编 "use of uninitialized variable 'a'"（line 7）；neg2（无初始化 Tuple）解释器 `neg2: (1, 2)` vs codegen 拒编 "variable declaration without initializer"（line 2，范围外）；ctest -C Release 差分 53/53，Debug 门禁 6/6
    - 范围外：无初始化 Tuple（形状无从推断需延迟槽组）维持拒编，动态域（数组过签名/字段/返回值）obj/kind≥2 元素读出维持 CG9 陷阱，分支/循环深层块赋值后读维持拒编不错编（同 t92）；零新增 collie_rt 接口
- [x] 父类静态类型调子类特有方法/字段（t102，S39 残余面解锁）
    - 范围拍板：候选盘点（无初值类字段 A / tuple 进函数签名 B / 父类静态类型调子类特有成员 C / 方法重载 D）——A 经 t93 排除（字段跨实例不可跟踪 uninit，静态布局无 none 表示）、B 需形状单态化架构级方案、D 解释器后定义者胜 vs 语义层 find_best_overload 择优两端语义不一致；C 胜出：S39 明示残余面（README L479 "拒编不错编陷阱面"），双端预检 4 探针实证——p1（父类静态调子类方法）解释器 `dog-only-rex`/`...` vs codegen 拒编 "undefined method 'only' on class 'Animal'"（活跃差分面）、p2（字段读）`husky` vs 同款拒编、p3（动态类无该方法）解释器运行期 "Runtime error at line 17, column 9: Undefined method 'only' on object" 确认陷阱消息、p4（类方法体内嵌套函数）`43` vs 拒编 "nested function declaration"（S44 范围外不选）；机制最就绪（t86 对象头类 id + switch 分派 + base-first 前缀布局），单任务覆盖 3 触点
    - 实现：collie_rt.c 新增 2 陷阱接口 `collie_rt_trap_undefined_method(name)` / `collie_rt_trap_undefined_property(name)`（stderr 消息核心对齐解释器 "Undefined method/property 'X' on object" + exit(1)，同 CG9 风格，头部注释登记）；code_generator.h 新增 2 成员声明；code_generator.cpp 四处——注册区 getOrInsertFunction 同 L117 模式；visitMethodCall 未命中分支（原 L1448 拒编）：收集 dispatch 含该方法的后代定义者（dispatch 表继承拷贝保证更深后代也有，按 id 排序），空维持拒编、非空先签名一致防御（ret/param 的 type+cls 全等，否则拒编 "inherited method with a different signature"）+ arity + 实参 coerce（签名以首个定义者为准）+ 读头部 id switch（case 各定义者调单态化实例 collie.<分派类>.<定义类>.<方法名>，default=陷阱块 CreateCall + CreateUnreachable）+ 非 Void 返回值 PHI 合流；visitProperty 未命中分支（原 L1632 拒编）：field_index 含该字段的后代定义者收集 + 字段类型一致防御（type/cls/bit_max 全等，否则拒编 "inherited field with a different type"）+ switch 各定义者字段 GEP（下标 = field_index + 1 跳头部，base-first 前缀布局下各定义者下标一致天然安全）+ default 陷阱 + PHI 合流；visitPropertyAssign 未命中分支（原 L1679 拒编）：同收集/防御，值在 switch 前求值/coerce/bit 范围陷阱（求值顺序 object→value 不变，对齐解释器），各 arm 按定义者布局 store，default 陷阱
    - 验证：p1/p2 探针编译产物输出与解释器逐字节一致（fc 无差异）；p3 陷阱实证——产物运行 exit 1 + stderr "runtime error: Undefined method 'only' on object"（消息核心对齐解释器，位置前缀缺失同越界陷阱既定分歧）；新差分用例 s55_upcast_members（三级链 Animal←Dog←Puppy：Dog/Puppy 双定义者 case 分派 fetch、带参特有方法 fetch_times（arity/coerce）、特有字段读、特有字段写（赋值表达式值+经自身类型回读）、深链特有成员 rollover/age 经 Animal 槽调用读写、返回值 upcast 后调特有成员、特有成员+覆写+父类字段混调、槽 upcast 写后互不影响、静态类实例回归 sound()→...，22 行输出双端逐字节一致）；ctest -C Release 差分 54/54（含 s55 新增），Debug 门禁 6/6
    - 范围外：downcast/无关类互赋维持拒编（需运行期类型校验），混合类实例数组字面量/整槽写异类维持拒编，方法重载维持拒编（两端语义不一致），无初值类字段维持拒编（t93 排除），tuple 进函数签名后置（架构级），类方法体内嵌套函数维持拒编（S44 范围外）；collie_rt 新增 2 个陷阱接口
- [x] 同一继承树内 downcast 族解锁（t103，S39 残余面）
...
- [x] 实例数组单槽写同继承树互赋 upcast/downcast（t118，t100 残余拒编面，S67）
    - 方案：visitIndexAssign Obj 元素守卫（原仅放行 upcast `is_subclass_of(v.cls, object.cls)`，t100）镜像整体互赋 t115 L1138——追加 `!is_subclass_of(object.cls, v.cls)` 才拒，同树任一方向放行、rt_arr_set 直存位模式（单槽写是运行期写读共享底层存储，无静态形状错配，无需 t117 类守卫——这是与整体互赋/换形状重赋的语义差异点）；同树内元素类沿链前缀布局兼容，d[0].field 按 object.cls（声明元素类）偏移解码、方法按对象头类 id 动态分派；兄弟类/无关类（互不为子类）特有字段偏移错位可能错值，保守拒编不错编；零新增 collie_rt 接口
    - 验证：p1/p2/p3 探针（上溯回归、下溯核心解锁均 IDENTICAL；兄弟类 Dog←Cat 保守拒编——解释器可跑而 codegen 拒编，拒编不错编）；新差分用例 s71_index_assign_cast（上溯单槽写回归/下溯单槽写/同树链内多轮单槽写/引用别名单槽写/函数内局部含 if 分支内/整体互赋+单槽写混用/循环内交替单槽写，19 行输出双端逐字节一致）；ctest -C Release 差分 70/70（含 s71 新增），单元 4/4，CLI 2/2
    - 范围外：兄弟类/无关类单槽写维持拒编不错编（特有字段偏移错位可能错值）；下溯写入后访问子类特有字段属解释器运行期错误边界（语义层静态类型仍为声明类，解释器按对象真实类查字段表运行期报错，codegen 按声明类偏移解码——维持既有降级语义，用例只访问父类级成员）；零新增 collie_rt 接口
- [x] 混合类实例数组字面量 NCA 收敛（t119，t100 残余拒编面，S68）
...
- 2026-08-01 `feat(compiler)`: codegen 无初始化 array/类类型变量声明（t101，M6）：Search 子代理枚举 code_generator.cpp 全部约 143 处 unsupported() 触点 + 对照 README 后置候选核实候选 A-E——候选 D-array（无初始化 `array` 变量）+ D-class（无初始化类类型变量）胜出（双端预检：p1 `array a; a=[1,2,3]; ...` 解释器 `arr: [1, 2, 3] 3`/`idx: 100 3` vs codegen 拒编 "variable declaration without initializer"；p2 `Cat c; c=new Cat("mimi"); ...` 解释器 `cat: mimi mimi meow` vs codegen 同拒编——活跃差分面，均复用 t92/t96 uninit + decl_depth 机制，D-array 叠加 t70 Num 动态域哨兵）；code_generator.cpp 三处——visitVarDecl 无初始化分支在静态类型放行块后、终点 unsupported 前加两分支：KW_ARRAY（create_var_slot(llvm_type_of(Arr)) opaque ptr 槽 + CGVar.type=Arr/elem=Num 动态域哨兵/uninit/decl_depth，后续赋值走 visitAssign Arr 动态域透传 t70、读出 kind 驱动、kind≥2 落 CG9 陷阱），IDENTIFIER 且 classes_.count(类名)（create_var_slot(llvm_type_of(Obj)) + CGVar.type=Obj/cls=类名/uninit/decl_depth，后续赋值走 visitAssign Obj 同类或子类 upcast t86）；visitAssign Arr 分支与 Obj 分支 CreateStore 后各补同块 uninit 清除 `if (var->uninit && scopes_.size()==var->decl_depth) var->uninit=false;`（两分支原 return 早退不经通用路径 t92 清除逻辑，不补则声明后同块赋值仍被 visitIdentifier uninit 守卫拒读）；同块赋值后放行读、深层块（分支/循环体）赋值后读保守拒编（同 t92 流不敏感语义）；范围外：无初始化 Tuple（形状无从推断需延迟槽组）维持拒编，动态域 obj/kind≥2 数组元素读出维持 CG9 陷阱；零新增 collie_rt 接口；新差分用例 s54_uninitvar（无初始化 array 隔句赋值 int 数组 print/len/正负索引读写、赋 decimal 数组混合表示、别名引用语义写联动、无初始化类变量赋值后字段读+方法调用、类变量别名写字段可见、无初始化父类变量接子类实例 upcast 后方法动态分派 sound()→woof、函数内局部无初始化 array+class、for 循环体内声明+同块赋值+同块读，12 行输出双端逐字节一致）+ 两实证（neg1 无初始化 array 仅在 if 块内赋值后读——解释器 `[1, 2, 3]` vs codegen 拒编 "use of uninitialized variable 'a'" 保守面；neg2 无初始化 Tuple——解释器 `(1, 2)` vs codegen 拒编 "variable declaration without initializer" 范围外），ctest -C Release 差分 53/53 逐字节一致，Debug 门禁 6/6（M6 t101）
- 2026-08-01 `feat(compiler)`: codegen 类实例进数组（同类，kind 5）（t100，M6）：前序双后置候选（异质数组字面量 A / 实例进数组 B / tuple 进函数签名 C）经 Search 触点核实 + 双端预检——候选 B 胜出（qb `array arr=[d1,d2]; print(len(arr))` 解释器 2 vs codegen 拒编 "array element type"，与 t88/t89 同构：本地 array 变量保留 elem=Obj 走静态 bits_to_elem 路径可支持读出+字段+方法）；code_generator.cpp 五处——三 helper 加 Obj 分支（elem_to_bits→PtrToInt / bits_to_elem→IntToPtr / arr_kind_of→5），visitArrayLiteral 局部 elem_cls 追踪首元素类名 + 全 Obj 元素混合类守卫（v.cls != elem_cls 拒编 "heterogeneous array literal (mixed classes)"）+ last_value_ 携 elem_cls，visitVarDecl array 分支拷贝 init.cls 入 CGVar、visitAssign 整数组重赋值补 Obj 异类守卫 + last_value_ 携 cls，visitIndex 静态读出 last_value_ 追加 object.cls 传播（identifier load 已携 var->cls，支持 arr[i].field/arr[i].method() 且方法按对象头类 id 动态分派），visitIndexAssign 静态写路径末补 Obj cls 兼容守卫（同类或子类 upcast 放行同 coerce_call_arg，异类拒编）；collie_rt.c 三触点加 kind 5——arr_to_str case 5 `<object>`（对齐 Value::to_string Instance）、arr_eq 同 kind 5 恒返 0（对齐解释器 values_equal 无 Instance 分支实例恒不等，含同实例/同数组自比较）、trap_arr_kind kind 5→`object`（动态域读出兜底）+ 两处头部注释登记；`==`/print 按 Arr 类型泛化下沉无 elem 守卫天然覆盖；消费面 build/len/print/toString/==/整槽写/本地静态读出全解锁，数组过签名/字段/返回值的动态域 obj 读出落 CG9 陷阱不错编；范围外：混合类实例数组字面量/整槽写异类维持拒编，动态域 obj 元素读出维持陷阱；零新增 collie_rt 接口签名；新差分用例 s53_objectarray（同类实例数组 build、len→2、print/toString→`[<object>, <object>]`、zoo[i].name 字段读 + zoo[0].describe()/sound() 方法调用、负索引 zoo[-1]、整槽写同类、子类 upcast 整槽写后读出方法动态分派 describe()→`rex says woof`、== 实例恒不等 neq/ne-true/self-neq，16 行输出双端逐字节一致）+ 两实证（混合类字面量 `[a, b]` 解释器 2 vs codegen 拒编 "heterogeneous array literal (mixed classes)"；obj 数组过函数签名后 `a[0]` 解释器 `<object>` vs codegen 产物运行期 trap "reading object array element in dynamic context ... gap CG9"），ctest -C Release 差分 52/52 逐字节一致，Debug 门禁 6/6（M6 t100）
- 2026-08-01 `feat(compiler)`: codegen 类方法/构造器 byte/word 参数与返回（t99，M6）：t97/t98 双后置候选解锁——方法注册循环两触点前置识别（返回类型 KW_BYTE/KW_WORD → CGType::Int + info.ret_bit_max=255/65535 复用 CGMethod 继承的 t97 ret_bit_max 字段；参数 KW_BYTE/KW_WORD → CGType::Int，llvm 形参 i64，均绕过 declared_signature_type 避免 "variable type 'byte/word'" 拒编）；gen_method_body 两触点——现场保存/复位加 current_ret_bit_max_（设 method.ret_bit_max、复位 0，visitReturn t97 陷阱自动生效覆盖方法/构造器返回越界）+ 形参落槽绑定处读 stmt 形参声明类型（param.type），byte/word 时先 check_bit_range（越界调 rt_trap_bit_range + unreachable）再 store 并置 CGVar.bit_max（体内重赋走赋值点陷阱）；关键差异（对照 t98 顶层函数）：方法/构造器为单签名按名解析无重载拦截，整数字面量实参（`c.addb(200)`）可达绑定点，解释器绑定时 coerce_to_declared 校验、越界字面量运行期 trap，故形参必须在绑定点插范围陷阱（统一覆盖方法/构造器/base 全调用路径），无需新增 param_bit_max 向量（绑定点直接由 AST 形参类型判别）；coerce_call_arg 调用点零改动；范围外：异质数组字面量/实例进数组/tuple 进函数签名维持拒编（活跃差分面后置候选），方法调用结果参与 ==/!= 比较、word→byte 返回属解释器语义边界（方法调用静态类型为 object、is_comparable_type 无 object 放行；word 值不能 return 给 byte 返回类型）非 codegen 拒编面；零新增 collie_rt 接口；新差分用例 s52_bytewordmethod（构造器 byte 参数字面量实参、方法 byte 参数字面量+变量实参、方法 byte 返回 print/算术/存变量、byte 参数+byte 返回多路径 pickByte、word 参数+word 返回多路径 clampWord、循环内累计 addTotal、方法返回值作方法实参、byte 返回值有序比较 >=，13 行输出 10/15/35/35/135/35/200/0/255/200/60000/36/ok）+ 两实证（构造器 `new C(300)`/方法 `c.setb(300)` 越界字面量实参——解释器 trap "Value out of range for 'byte' (expected 0-255, got 300)" vs codegen 产物运行期 trap 核心消息对齐，均走绑定点 check_bit_range），ctest -C Release 差分 51/51 逐字节一致，Debug 门禁 6/6（M6 t99）
- 2026-08-01 `feat(compiler)`: codegen byte/word 函数参数（t98，M6）：declare_function 顶层/嵌套函数参数循环 KW_BYTE/KW_WORD 前置识别为 CGType::Int 承载（绕过 declared_signature_type 避免 "variable type 'byte/word'" 拒编，llvm 形参 i64）；visitFunction 形参落槽绑定改用具名 CGVar binding，byte/word 形参置 bit_max=255/65535——体内重赋走既有 visitAssign 赋值点 check_bit_range 陷阱、返回越界走 visitReturn t97 陷阱；关键预检证伪 t97 遗留假设：p1/p2/p4 探针（整数字面量 `addb(200)`/`addb(300)`、byte 算术 `addb(x+100)` 实参）双端均报语义错 "No matching overload"——重载解析要求实参类型即 byte/word，故实参只能是 byte/word 变量（声明/赋值点已校验）或 byte/word 函数返回（visitReturn t97 已校验），恒在范围内，coerce_call_arg 调用点零改动无需范围陷阱（类型系统天然保证）；修正 t97 返回类型块过时注释；范围外：类方法/构造器 byte/word 参数（走 declared_signature_type 维持拒编，活跃差分面后置）、整数字面量/算术实参非差分面；异质数组/实例进数组/tuple 进签名后置；零新增 collie_rt 接口；新差分用例 s51_bytewordparam（byte 形参 identity inc、clamp 多返回、word 形参 addw、嵌套调用 chain、byte 函数返回作实参 inc(makeByte())、循环内反复以 byte 变量为实参、形参返回值 == 比较、word 返回参与算术，9 行输出 51/50/200/60001/51/101/3/ok/106）+ 实证（p6 形参体内重赋越界 `b = b + 100` 实参 200——解释器 trap "Value out of range for 'byte' (expected 0-255, got 300)" vs codegen 产物运行期 trap 核心消息对齐），ctest -C Release 差分 50/50 逐字节一致，Debug 门禁 6/6（M6 t98）
- 2026-08-01 `feat(compiler)`: codegen byte/word 函数返回类型（t97，M6）：declare_function 顶层函数返回类型 KW_BYTE/KW_WORD 前置识别为 CGType::Int 承载 + CGFunction 新增 ret_bit_max（255/65535，缺省 0 保聚合初始化兼容，绕过 declared_signature_type 避免 "variable type 'byte/word'" 拒编），functions_ 初始化带入 ret_bit_max；code_generator.h 加 current_ret_bit_max_ 成员，visitFunction 沿 current_ret_type_ 保存/设置/恢复模式（嵌套函数上下文安全）；visitReturn 值对齐 Int 后 current_ret_bit_max_>0 时插 check_bit_range（复用 t69 既有陷阱，(u64)v > max 一次覆盖负数与超上限，越界调 rt_trap_bit_range 报 "Value out of range for 'byte/word'" + unreachable，对齐解释器 coerce_to_declared 返回值校验，多返回路径逐 return 各插）；返回值消费端为普通 Int 表达式域（存 byte/word 变量再走声明/赋值点 check_bit_range、print 直接整数输出）；范围外：byte/word 作函数参数（活跃差分面，后置——需 CGFunction per-param bit_max + visitFunction 绑定置 CGVar.bit_max + coerce_call_arg 调用点范围陷阱）、类方法返回 byte/word（走 declared_signature_type 维持拒编）；异质数组字面量/实例进数组/tuple 进函数签名三活跃差分面维持后置（t95a/t96a 已实证）；零新增 collie_rt 接口；新差分用例 s50_bytewordret（byte/word 返回值 print、参与算术、存 byte/word 变量走声明与赋值点范围陷阱、clampToByte 多返回路径、循环内反复调用、word 返回值 == 比较，12 行输出 200/60000/210/200/255/255/42/60100/200/201/202/wordok）+ 两实证（越界 word 返回探针 p3 `return 70000` codegen 产物运行期 trap "Value out of range for 'word' (expected 0-65535, got 70000)" 核心消息对齐解释器、byte 作函数参数探针 p1 `addb(b byte)` 解释器 201 vs codegen 拒编 "variable type 'byte'" 活跃差分面后置实证），ctest -C Release 差分 49/49 逐字节一致，Debug 门禁 6/6（M6 t97）
- 2026-08-01 `feat(compiler)`: codegen 无初始化变量声明放行面扩展（t96，M6）：visitVarDecl 无初始化分支从四静态类型 {integer,decimal,bool,string}（t92）扩展到 {number,tribool,byte,word,char,character}——byte/word 前置分支创建 i64 槽 + CGVar.bit_max（255/65535）+ uninit（赋值走 visitAssign 通用路径，既有赋值点 check_bit_range 范围陷阱自动生效，不丢范围校验），number/tribool/char/character 并入静态类型放行条件（Num struct{i64,i64} / Tri i8 / Str ptr 零初始化 GlobalVariable/alloca 均合法常量，uninit 期读拒编保证零值不可观测）；uninit/decl_depth/同块清除/链底快照机制全部复用 S45 t92 无新增控制流；选型排除记录：异质数组字面量/实例进数组/tuple 进函数签名三活跃差分面维持后置（t95a 已实证）；范围外：array（elem 无从推断）/Tuple（形状无从推断）/类类型无初始化维持拒编，分支/循环块内赋值后读维持拒编不错编；零新增 collie_rt 接口；新差分用例 s49_uninitmore（number 整/小数态、tribool 三态、byte/word 范围内赋值、char/character、函数内局部 number、顶层全局 tribool 函数体内读、循环体内声明+同块赋值 number，16 行输出）+ 两实证（array 无初始化维持拒编、byte 分支块内赋值后读维持拒编——解释器可跑 vs 拒编 "use of uninitialized variable"），ctest -C Release 差分 48/48 逐字节一致，Debug 门禁 6/6（M6 t96）
