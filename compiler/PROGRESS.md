...
最后更新：2026-07-29（t83 完成：codegen 非常量 tuple 索引——同质 tuple（元素同 CGType 且 ∈ {Int/Double/Bool/Str}）的 `t[i]`（i 为变量/表达式）物化为运行时数组后 rt_arr_get(动态 idx) 取值，复用负索引归一化 + 越界陷阱（消息 "Index N out of range (size M)" 与解释器一致），零新增 rt 接口，差分 35/35）
